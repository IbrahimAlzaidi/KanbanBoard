...
class DbHelper(context: Context) : SQLiteOpenHelper(context,DBNAME,null,DBVERSION) {
    ...
    @SuppressLint("Range")
    fun getAllTasksTitle(status : String):MutableList<DbTaskModel>{
        val tasksList : ArrayList<DbTaskModel> = ArrayList()
        val cursor : Cursor? = readableDatabase.rawQuery("SELECT * FROM ${DbSchema.TABLE_TASKS} WHERE ${DbSchema.TASK_TITLE} = ?", arrayOf(status))
        var idTask : Int?
        var titleTask : String?
        var descTask:String?
        var statsTask:String?
